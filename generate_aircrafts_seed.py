...
import os
from typing import Optional   # (only for the type hint below)
...
def generate_aircraft_instances(
    ...
    max_turnaround_minutes: int = 1680,   # 28 hrs → min
    seed: Optional[int] = None            # ← NEW: makes randomness reproducible
):

    if seed is not None:
        random.seed(seed)     # Python’s built-in RNG
        np.random.seed(seed)  # NumPy’s RNG (future-proofing)
    ...
    def generate_single_instance(instance_id: int):
        while True:                      # keep generating until constraints met
            random_day = datetime.strptime(
                f"2025-{random.randint(1,12):02d}-{random.randint(1,28):02d}",
                "%Y-%m-%d"
            )

            aircraft_data, used_wps, total_man_hours = [], set(), 0.0
            ...
            while len(aircraft_data) < num_aircrafts:
                ...
                selected_wps, total_minutes, total_hours = [], 0, 0.0
                ...
                aircraft_data.append({
                    ...
                    "Work that needs to be carried out": ", ".join(selected_wps)
                })

            if (min_total_man_hours < total_man_hours < max_total_man_hours
                    and set(all_wp_numbers).issubset(used_wps)):
...
if __name__ == "__main__":
    generate_aircraft_instances(
        ...
        num_instances=10,
        seed=42                     # ← pass any integer you like
    )