...
def generate_aircraft_instances(
    ...
    """
    Generates CSVs where:
      - every WP is covered at least once per instance
      - WPs can repeat to hit the 70–90% man-hour window
      - each turnaround = exactly scaling_factor * work-minutes
      - retries until all constraints are met or max_attempts is reached
    """
    ...
    # 2) Load & normalize work packages
    ...
    def generate_single_instance(instance_id: int):
        ...
        for attempt in range(1, max_attempts + 1):
                ...
                aircraft_data.append(
                    {
                        ...
                        "Work that needs to be carried out": ", ".join(sel_wps),
                    }
                )
            ...
            if total_man_used < raw_min_man or total_man_used > raw_max_man:
                manhour_fails += 1
                continue
            ...
            fn = os.path.join(output_dir, f"aircrafts_instance_{instance_id}.csv")
